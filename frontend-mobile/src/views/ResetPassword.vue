      ...
      <div class="auth-card initial-load" ref="authCard">
        <div class="view-section active">
          ...
          <div v-if="step === -1" class="error-panel">
            ...
            <h3 class="success-title">链接无效</h3>
            <p class="success-desc">重置密码链接无效，请重新申请</p>
            <button class="btn-primary" @click="goToLogin" style="margin-top: 8px;">返回登录</button>
          </div>
        </div>
      </div>
    </div>
...
async function handleSubmit() {
  ...
  try {
    ...
    step.value = 2
    vantShowToast({ message: '密码重置成功，请登录', type: 'success' })
  } catch {
    // 错误已在拦截器处理
  } finally {
    loading.value = false
