    ...
    <div class="qr-overlay" v-if="visible" @click.self="handleClose">
      <div class="qr-modal">
        ...
        <div class="qr-body" v-else-if="status === 'expired'">
          <div class="qr-icon-wrap qr-error-icon">
            <span>&#9888;</span>
          </div>
        ...
        <div class="qr-body" v-else-if="status === 'need_reg'">
          <div class="qr-icon-wrap qr-success-icon">
            <span>&#10003;</span>
          </div>
  ...
  const poll = async () => {
    ...
    try {
      ...
      if (res.status === 'success') {
        userStore.qrLogin({ token: res.token, user: res.user })
        visible.value = false
        router.push('/')
        return
        ...
        setTimeout(() => {
          ...
          router.push({
            name: 'Register',
            query: { student_id: res.student_id, name: res.name }
          })
        }, 1500)
        return
...
function startCountdown() {
  countTimer = setInterval(() => { countdown.value-- }, 1000)
}
...
watch(visible, (val) => { if (val) start() })
...
.qr-error-icon { background: rgba(255, 77, 79, 0.2); color: #ff4d4f; }
.qr-success-icon { background: rgba(82, 196, 26, 0.2); color: #52c41a; }
