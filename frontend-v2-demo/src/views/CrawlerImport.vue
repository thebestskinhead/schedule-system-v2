  ...
  <div class="main-container">
    <div class="card">
      <div class="card-header">
        <span class="card-title">自动导入无课表</span>
        <el-button @click="$router.push('/availability')">返回无课表</el-button>
      </div>

      <!-- 步骤说明 -->
      <el-steps :active="currentStep" simple class="mb-4">
      ...
      <!-- 输入表单 -->
      <div v-if="!isImporting && !importResult">
      ...
      <!-- 预览结果 -->
      <div v-if="previewData.length > 0 && !isImporting" class="mt-4">
        <div class="card-header">
          <span class="card-title">课表预览（共 {{ previewData.length }} 周）</span>
        </div>
      ...
      <!-- 导入中状态 -->
      <div v-if="isImporting" class="mt-4 importing-card">
        <div class="card-header">
          <span class="card-title">正在导入无课表</span>
        </div>
...
.preview-timetable th,
.preview-timetable td {
  border: 1px solid #e5e5e5;
  text-align: center;
