    ...
    <div class="container">
      <div class="auth-card initial-load" ref="authCard">
        <div class="view-section active">
          ...
          <div v-if="step === 0" class="loading-panel">
            <div class="loading-text">正在验证链接有效性...</div>
          </div>

          <form v-if="step === 1" @submit.prevent="submitReset">
          ...
          <div v-if="step === 2" class="success-panel">
            ...
            <p class="success-desc">您可以使用新密码登录系统了</p>
            <button class="btn-primary" @click="goToLogin" style="margin-top: 8px;">去登录</button>
          </div>

          <div v-if="step === -1" class="error-panel">
            <div class="success-icon" style="background: rgba(255,100,100,0.2);">!</div>
            <h3 class="success-title">链接无效或已过期</h3>
            <p class="success-desc">重置密码链接已过期或无效，请重新申请</p>
            <button class="btn-primary" @click="goToForgot" style="margin-top: 8px;">重新申请</button>
          </div>
        </div>
      </div>
    </div>
